...
$stmt = $con->prepare("
    ...
    WHERE p.id = ? AND p.jenis='ruangan' AND p.status=''
");
...
// Ubah ruangan jadi tersedia
$con->query("UPDATE ruangan SET status='tersedia' WHERE id=".$data['id_item']);
...
class PDF2 extends FPDF {
    function Header() {
        ...
        $this->Cell(0,6,'BERITA ACARA PENGEMBALIAN RUANGAN',0,1,'C');
        $this->Ln(2);
    }
}
...
$pdf->MultiCell(0,6,"Pada hari ini ".tanggalIndo(date('Y-m-d'))." telah dilakukan pengembalian ruangan dari:",0,'J');
$pdf->Ln(3);
...
$pdf->SetFont('Arial','',11);
$pdf->MultiCell(0,6,
"Telah mengembalikan kruangan berikut:\n\n".
...
$pdf->Ln(5);
$pdf->MultiCell(0,6,
"Ruangan telah diterima kembali dalam kondisi baik. Jika ditemukan kerusakan, maka akan ditindaklanjuti sesuai ketentuan.",
0,'J');
...
$link = 'http://localhost/PinjamRuanganRuangan/pdf-kembali/'.$yearDir.'/'.$filename;
$msg = 'Pengembalian ruangan Anda telah disetujui. Klik untuk melihat/cetak Berita Acara: '.$link;
